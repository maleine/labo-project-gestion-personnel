...
class Formation {
  static async getAll() {
    ...
    const result = await pool.request().query(`
      SELECT 
        f.*,
        COUNT(DISTINCT iif.personnel_id) as nb_inscrits,
  ...
  static async create(data) {
    ...
    try {
      const result = await pool.request()
        .input('reference', sql.VarChar(50), data.reference)
        .input('titre', sql.VarChar(200), data.titre)
        .input('type_formation', sql.VarChar(50), data.type_formation)
        .input('organisme', sql.VarChar(200), data.organisme || null)
        ...
        .input('lieu', sql.VarChar(200), data.lieu || null)
        .input('description', sql.VarChar(sql.MAX), data.description || null)
        .input('objectifs', sql.VarChar(sql.MAX), data.objectifs || null)
        .query(`
          INSERT INTO Formations (
            reference, titre, type_formation, organisme, duree_heures, 
            date_debut, date_fin, cout, lieu, description, objectifs, statut
          )
          VALUES (
            @reference, @titre, @type_formation, @organisme, @duree_heures,
            @date_debut, @date_fin, @cout, @lieu, @description, @objectifs, 'Planifiée'
          );
